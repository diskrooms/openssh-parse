...
ask_filename(struct passwd *pw, const char *prompt)
{
	...
	if (key_type_name == NULL)
		...
		case KEY_RSA_CERT:
		case KEY_RSA:
			name = _PATH_SSH_CLIENT_ID_RSA;				//defined in the pathnames.h 	The value is ".ssh/id_rsa"
			break;
		case KEY_ED25519:
	...
	snprintf(identity_file, sizeof(identity_file),
	    "%s/%s", pw->pw_dir, name);							//The value of identity_file  is   "/root/.ssh/id_rsa"
...
main(int argc, char **argv)
{
	...
	*passphrase2;
	...
	__progname = ssh_get_progname(argv[0]);		//ssh-keygen
	//printf("%s\n", __progname);
#ifdef WITH_OPENSSL
	...
	snprintf(dotsshdir, sizeof dotsshdir, "%s/%s",	//写入dotsshdir 
	    pw->pw_dir, _PATH_SSH_USER_DIR);	/// _PATH_SSH_USER_DIR = .ssh    pw->pw_dir = /root

	//printf("%s\n", dotsshdir);			
	if (strstr(identity_file, dotsshdir) != NULL) {	//密钥文件在.ssh文件夹下
		if (stat(dotsshdir, &st) < 0) {											//检测.ssh文件夹状态
	...
	if (stat(identity_file, &st) >= 0) {												//检测密钥文件是否存在 存在提示是否覆盖原文件
		char yesno[3];
	...
	if (identity_passphrase)
		passphrase1 = xstrdup(identity_passphrase);								//xstrdup 位于xmalloc.c 对 malloc进行了封装  函数作用：复制字符串
	else if (identity_new_passphrase)
		passphrase1 = xstrdup(identity_new_passphrase);
	else {
passphrase_again:
	...
	if ((r = sshkey_save_private(private, identity_file, passphrase1,comment, use_new_format, new_format_cipher, rounds)) != 0) {
		error("Saving key \"%s\" failed: %s",
		    identity_file, ssh_err(r));
		explicit_bzero(passphrase1, strlen(passphrase1));
		free(passphrase1);
